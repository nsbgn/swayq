...
# Add `.occupancy`, `.windows` and `.insert` point to all subschemas on this
# level.
...
# (1) assign a partition of windows and (2) determine whether any newly opened
# window should appear there.
def _assign_windows($windows):
  .windows = $windows |
  if .subschemas then
    .subschemas |= (
...
def _assign_representative($container):
  if .subschemas then
    ...
    .representative = .windows[0]
  end;

# Assume a schema such that it and all its subschemas satisfy the following:
def assign($source_container):
  _assign_defaults |
  _assign_capacity |
  _assign_windows([$source_container | con::leaves]) |
  _assign_representative($source_container);
